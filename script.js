...
function calculateSLAStatus(returnDateStr) {
  if (!returnDateStr)
    return { label: "Sem Agendamento", class: "sla-neutral", days: 0 };
  ...
  if (daysDiff === 0)
    return { label: "Atender Hoje", class: "sla-today", days: 0 };
  if (daysDiff > 0 && daysDiff <= 1)
    return { label: "Atrasado +24h", class: "sla-warning", days: daysDiff };
  if (daysDiff > 1 && daysDiff <= 7)
    return {
      label: `Atrasado ${daysDiff} dias`,
      class: "sla-danger",
      days: daysDiff,
    };
  if (daysDiff > 7 && daysDiff <= 15)
    return {
      label: "Sem contato +15 dias",
      class: "sla-critical",
      days: daysDiff,
    };
  if (daysDiff > 15)
    return {
      label: "Sem retorno (+30 dias)",
      class: "sla-cold",
      days: daysDiff,
    };

  return {
    label: `Agendado (${formatDate(returnDateStr)})`,
    class: "sla-ok",
    days: daysDiff,
  };
}
...
function saveLead() {
  ...
  const activeView = activeNav
    ? activeNav.getAttribute("data-view")
    : "dashboard";
  showView(activeView);
}
    ...
    return `
            <div class="kanban-col" ondragover="allowDrop(event)" ondrop="drop(event, '${stage.id}')">
                ...
                <div class="kanban-cards">
                    ${
                      stageLeads.length === 0
                        ? '<p class="kanban-empty">Sem leads nesta etapa</p>'
                        : stageLeads
                            .map(
                              (l) => `
...
function closeLeadModal(event) {
  if (
    ...
    event.target.className === "btn-secondary"
  ) {
...
function openDetailModal(id) {
  ...
  body.innerHTML = `
        ...
        <div>
            ...
            ${
              historico.length === 0
                ? '<p style="color:#777; font-size:0.85rem;">Nenhuma nota registrada até o momento.</p>'
                : `
